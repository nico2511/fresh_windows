...
function Get-SyncLocalScriptsPowerShellCommand {
    ...
    return @"
`$ErrorActionPreference='Stop'; try { [Net.ServicePointManager]::SecurityProtocol = [Net.ServicePointManager]::SecurityProtocol -bor [Net.SecurityProtocolType]::Tls12 } catch {}; `$ref='$RepoRef'; `$fresh=Join-Path `$env:LOCALAPPDATA 'FreshWindows'; New-Item -ItemType Directory -Path `$fresh -Force | Out-Null; `$raw='https://raw.githubusercontent.com/nico2511/fresh_windows/'+`$ref; foreach (`$n in @('GameMode-Common.ps1','Invoke-GameModeKill.ps1','GameMode-WatchAgent.ps1')) { Invoke-WebRequest -Uri (`$raw+'/scripts/'+`$n) -OutFile (Join-Path `$fresh `$n) -UseBasicParsing }; `$agentScripts=@('lib/FreshAgent-Config.ps1','lib/FreshAgent-SkillsEngine.ps1','lib/FreshAgent-SkillHandlers.ps1','lib/FreshAgent-GameSession.ps1','ai/Ollama-Manager.ps1','ai/Windows-Stt.ps1'); foreach (`$rel in `$agentScripts) { `$dest=Join-Path `$fresh (`$rel -replace '/','\'); New-Item -ItemType Directory -Path (Split-Path `$dest -Parent) -Force | Out-Null; Invoke-WebRequest -Uri (`$raw+'/scripts/'+(`$rel -replace '\\','/')) -OutFile `$dest -UseBasicParsing }; `$cfgPaths=@('configs/agent-ai.json','configs/skills-apps.json','configs/skills-web.json','configs/skills/registry.json'); foreach (`$rel in `$cfgPaths) { `$dest=Join-Path `$fresh (`$rel -replace '/','\'); New-Item -ItemType Directory -Path (Split-Path `$dest -Parent) -Force | Out-Null; Invoke-WebRequest -Uri (`$raw+'/'+(`$rel -replace '\\','/')) -OutFile `$dest -UseBasicParsing }; Set-Content -LiteralPath (Join-Path `$fresh 'scripts.ref') -Value `$ref -Encoding UTF8 -NoNewline
"@
}
